                        ...
                        <div className="p-4">
                            <div className="flex items-center justify-between mb-4">
                                <h2 className="text-lg font-bold text-gray-900">
                                    {kpiCards.find(kpi => kpi.title.toLowerCase().replace(' ', '_') === selectedSensor)?.title} History
                                </h2>
                                <button
                                    onClick={() => setSelectedSensor(null)}
                            ...
                            <div className="h-96">
                                <Line 
                                    ...
                                    data={{
                                        labels: historicalData.labels,
                                        datasets: [{
                                            label: kpiCards.find(kpi => kpi.title.toLowerCase().replace(' ', '_') === selectedSensor)?.title,
                                            data: historicalData.datasets[selectedSensor],
                                            borderColor: kpiCards.find(kpi => kpi.title.toLowerCase().replace(' ', '_') === selectedSensor)?.color.replace('text-', '#').replace('-600', ''),
                                            backgroundColor: `${kpiCards.find(kpi => kpi.title.toLowerCase().replace(' ', '_') === selectedSensor)?.color.replace('text-', '#').replace('-600', '')}1A`,
                                            tension: 0.4,
                                            fill: true,
                                            borderWidth: 2
                                        }]
